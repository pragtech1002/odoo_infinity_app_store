                ...
                for content in contents:
                    if content['type'] == 'dir' and content['name'] != '__pycache__':
                        ...
                        if index_html_response.status_code == 200:
                            index_html_data = index_html_response.content
                            index_html_filename = f"{module_name}_index.html"
                            index_html_attachment = self.env['ir.attachment'].create({
                                'name': index_html_filename,
                                'type': 'binary',
                                'datas': base64.b64encode(index_html_data),
                                'res_model': 'product.template',
                                'res_id': self.id,
                            })
                            index_html_path = f"/web/image/{index_html_attachment.id}/index_html_image"
                        ...
                        if manifest_response.status_code == 200:
                            ...
                            vals = {
                                ...
                                'gif_image': gif_path,
                                'index_html_content': index_html_path,  # Store the path instead of content
                            ...
                            if product_product_data:
                                ...
                                self.env['product.product'].create({
                                    ...
                                    #     'attribute_id': odoo_version_attribute.id,
                                    #     'product_attribute_value_id': odoo_version_value.id
                                    # })],
                                    **vals
                                })
                        else:
